...
const ProposalPreview: React.FC<ProposalPreviewProps> = ({ clientName, phases, total, onBack }) => {
  ...
  return (
    <div className="max-w-5xl mx-auto py-10 px-4">
      ...
      <div className="mb-8 flex justify-between items-center no-print">
        ...
        <div className="flex gap-3">
          <button 
            ...
            className="px-5 py-2.5 bg-white border border-slate-200 text-slate-700 rounded-xl hover:bg-slate-50 font-bold text-sm flex items-center shadow-sm"
          >
            <svg className="w-4 h-4 mr-2 text-green-600" fill="currentColor" viewBox="0 0 24 24"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8l-6-6z"/></svg>
            Export Excel
          </button>
          <button 
            onClick={handleGeneratePDF}
            className="px-6 py-2.5 bg-blue-600 text-white rounded-xl hover:bg-blue-700 shadow-lg shadow-blue-200 font-bold text-sm flex items-center transition-all active:scale-95"
          >
      ...
      <div 
        ref={documentRef}
        className="bg-white shadow-[0_20px_50px_rgba(0,0,0,0.1)] border border-slate-100 rounded-sm mx-auto" 
        style={{ width: '800px', minHeight: '1130px' }}
      >
        <div className="p-16">
          {/* Header Section */}
          <div className="flex justify-between items-start border-b-4 border-blue-600 pb-12 mb-12">
            <div className="flex items-center space-x-5">
              <div className="w-20 h-20 bg-blue-600 rounded-2xl flex items-center justify-center shadow-lg shadow-blue-100">
                 <span className="text-white font-black text-5xl">O</span>
              </div>
              <div>
                <h1 className="text-4xl font-black text-slate-900 tracking-tighter uppercase">OSMOZ</h1>
                <p className="text-blue-600 text-xs font-black tracking-[0.2em] uppercase">Digital Real Estate Solutions</p>
              </div>
            </div>
            <div className="text-right">
              <div className="inline-block bg-blue-50 text-blue-700 px-4 py-1 rounded-full text-xs font-black mb-3">OFFRE COMMERCIALE</div>
              <div className="text-slate-400 text-xs font-mono uppercase">REF: OSM-2025-{Math.floor(Math.random() * 10000)}</div>
              <div className="text-slate-900 font-bold text-sm mt-1">{currentDate}</div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-12 mb-16">
            <div className="bg-slate-50 p-8 rounded-2xl">
              <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">De la part de</h3>
              <p className="text-xl font-black text-slate-900 mb-1">Osmoz Digital Services</p>
            ...
            <div className="border-2 border-blue-50 p-8 rounded-2xl">
              <h3 className="text-[10px] font-black text-blue-400 uppercase tracking-widest mb-4">Préparé pour</h3>
              <p className="text-xl font-black text-slate-900 mb-1">{clientName}</p>
              <p className="text-sm text-slate-500 italic mb-4">Secteur : Promotion Immobilière</p>
          ...
          <div className="mb-16 max-w-2xl">
            <h2 className="text-2xl font-black text-slate-900 mb-4">Objet de la Mission</h2>
          ...
          {/* Module Breakdown */}
          <div className="mb-16">
            <h2 className="text-lg font-black text-slate-900 mb-8 flex items-center">
              <span className="w-8 h-8 bg-blue-600 text-white rounded-lg flex items-center justify-center mr-3 text-sm">01</span>
            ...
            <div className="space-y-10">
              {selectedPhases.map((phase, idx) => {
                const opt = phase.selectedOption === PricingOption.STARTER ? phase.starter : phase.standardPlus;
                return (
                  <div key={phase.id} className="relative pl-8 border-l-2 border-slate-100 pb-2">
                    <div className="absolute -left-[9px] top-0 w-4 h-4 rounded-full border-4 border-white bg-blue-600 shadow-sm"></div>
                    
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <h4 className="text-lg font-black text-slate-900 leading-none mb-2">{phase.title}</h4>
                      ...
                      <div className="text-xl font-black text-slate-900 tracking-tight">{opt.price.toLocaleString()} DZD</div>
                    </div>

                    <div className="grid grid-cols-2 gap-x-8 gap-y-2">
                      {opt.features.map((f, i) => (
                        <div key={i} className="flex items-center text-[11px] text-slate-500 font-medium">
          ...
          {/* Financial Summary Box */}
          <div 
            className="bg-slate-900 text-white p-12 rounded-[2rem] mb-16 flex justify-between items-center relative overflow-hidden"
            style={{ backgroundColor: '#0f172a' }} // Safety for PDF capture
          >
            {/* Decorative background element */}
            <div className="absolute top-[-50%] right-[-10%] w-64 h-64 bg-blue-600 rounded-full blur-[100px] opacity-20"></div>
            
            <div className="relative z-10">
              <div className="text-blue-400 text-xs font-black mb-2 uppercase tracking-[0.2em]">Investissement Global</div>
              <div className="text-6xl font-black tracking-tighter">
                {total.toLocaleString()} 
                <span className="text-2xl font-light text-slate-400 ml-3 uppercase">DZD HT</span>
              </div>
            </div>
            
            <div className="text-right relative z-10 border-l border-slate-700 pl-12">
              <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-4">Modalités</h4>
              <div className="space-y-2 text-xs font-bold text-slate-300">
                <p className="flex justify-between gap-8"><span>Acompte :</span> <span className="text-white">30%</span></p>
                <p className="flex justify-between gap-8"><span>Livraison :</span> <span className="text-white">40%</span></p>
                <p className="flex justify-between gap-8"><span>Recette :</span> <span className="text-white">30%</span></p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-24 pt-12 border-t border-slate-100">
            <div>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-16">Le Prestataire (Osmoz)</p>
              <div className="h-24 border-b-2 border-slate-100 w-full mb-3 flex items-end pb-2">
                <span className="text-slate-300 font-serif italic text-sm">Cachet et Signature</span>
              </div>
              <p className="text-[10px] text-slate-400">Fait à Alger, le {currentDate}</p>
            </div>
            <div>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-16">Le Client ({clientName})</p>
              <div className="h-24 border-b-2 border-slate-100 w-full mb-3 flex items-end pb-2">
                <span className="text-slate-300 font-serif italic text-sm">Mention "Bon pour accord"</span>
              </div>
              <p className="text-[10px] text-slate-400">Nom et qualité du signataire</p>
            </div>
          </div>

          <div className="mt-24 text-center">
            <p className="text-[9px] text-slate-400 italic mb-1 uppercase tracking-tighter">
